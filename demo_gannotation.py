...
import cv2
import torch
myGAN = GANnotation.GANnotation(path_to_model='training/Exp_2/model_4.gen.pth')
...
out = cv2.VideoWriter('test_3.avi',fourcc, 30.0, (128,128))
for imout in images:
    out.write(cv2.cvtColor(imout, cv2.COLOR_RGB2BGR))

out.release()
np.savetxt('test_3_cropped.txt', cropped_pts.reshape((132,-1)).transpose())
