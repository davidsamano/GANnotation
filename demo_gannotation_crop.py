...
image = cv2.cvtColor(cv2.imread('test_images/test_2.jpg'),cv2.COLOR_BGR2RGB)
image, points = utils.crop(image, points, size=128, tight=16)
...
out = cv2.VideoWriter('test_2.avi',fourcc, 30.0, (128,128))
for imout in images:
    out.write(cv2.cvtColor(imout, cv2.COLOR_RGB2BGR))

out.release()
np.savetxt('test_2_cropped.txt', cropped_pts.reshape((132,-1)).transpose())
